					...
					<div class="contents_wrap_01 info_box_container">
						...
						<tbody>
							...
							<tr>
								...
								<td>아이디를 찾지 못했습니다. (관리자에게 문의하세요.)</td>
								</c:if>
						...
						<form action="findAccount" id="findAccount">
							<table>
								<tr>
						...
						<div class="bottom_btn_log">
							...
							<button type="button" class="btn btn_navy1 btn_log" onclick="javascript:find()">찾기</button>
						</div>
					</div>	 
				</div>
			</div>
...
<script>
	...
	$(document).ready(function() {
		if($("#msg").val() == "fail"){
			alert("입력정보와 일치하는 아이디를 찾지 못했습니다.");
		}
	});
	...
	function find() {
		...
		if($("#userName").val() != "" && $("#birthdate").val() != "" && $("#email").val() != "") {
			$("#findAccount").submit();
		}
	}
